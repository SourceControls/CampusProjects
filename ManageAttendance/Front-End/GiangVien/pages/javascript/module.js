...
async function loadList(GV = {}, con) {
    ...
    for (let bh of newList) {
      ...
      <td class="align-middle">
        <button type="button" data-set="${
            bh.IDBUOIHOC + '&' + bh.TENMH + '&' + bh.NGAY
        }" class="btn btn-outline-success btn-xs mb-0 view btn-dd2" data-bs-toggle="modal" data-bs-target="#diemdanh">Điểm danh</button>
      </td>
    </tr>
          `;
    }
...
async function initEvent() {
        ...
        for (let i = 0; i < newList.length; i++) {
            if (isHappen(newList[i].TIETBD, newList[i].SOTIET)) {
                if (i == 0) {
                    ...
                    btnddHappening.dataset.set = `${
                        newList[i].IDBUOIHOC + '&' + newList[i].TENMH + '&' + newList[i].NGAY
                    }`;
                    mark += 1;
                }
                if (i > 0) {
    ...
    if (newList.length == 1) {
        if (isHappen(newList[0].TIETBD, newList[0].SOTIET)) {
            ...
            if (newList2[0])
                BHonAir1.innerText = `Môn học:\t${newList2[0].TENMH}\nNgày: ${newList2[0].NGAY}\nTiết bắt đầu: ${newList2[0].TIETBD}`;
            else BHonAir1.innerText = 'Không có buổi học';
            btnddHappening.style.display = 'none';
        }
    }
    if (newList.length == 0) {
