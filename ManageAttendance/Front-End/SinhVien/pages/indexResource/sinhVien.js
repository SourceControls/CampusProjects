...
var time = new Date().getFullYear() + '-' + currentMonth + '-' + currentDay
...
var loadLh = async function (data) {
    ...
    if (!id) {
        ...
        classHpn.innerHTML += `
            <button id="${list[0].IDLSV} ${
            list[0].IDBUOIHOC
        }" type="button" class="btn btn-success mb-0 w-100 h-100 border-radius-lg qrBtn" data-bs-toggle="modal" data-bs-target="#qr">
                    ...
                    <div class="text-sm font-weight-normal">${
                        gv2[0].HOTEN + ' - ' + gv2[0].SDT + ' - ' + gv2[0].EMAIL
                    }</div>
            </button>
        `
    }
    ...
    classHpn1.innerHTML += `
        ...
        <div class="text-sm font-weight-normal">${lhp1[0].TENLOP} - ${phong == '' ? list[1].PHONG : phong} - ${
        lhp1[0].TENMH
    }</div>
